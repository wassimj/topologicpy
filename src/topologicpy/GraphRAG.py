...
"""
    ...
    def SummarizeGraph(grag, graph, silent: bool = False) -> Dict[str, Any]:
        ...
        -------
        dict
            A JSON-friendly dictionary containing nodes, edges, and counts.
        """
        effective_silent = GraphRAG._effective_silent(grag, silent)
        ...
        if is_tgraph:
            try:
                vertices = TGraph.Vertices(graph) or []
                edges = TGraph.Edges(graph) or []
                nodes = []
                index_to_id = {}
                ...
                for i, v in enumerate(vertices):
                    ...
                    if vid is None or str(vid).strip() == "":
                        vid = f"n{i}"
                    label = props.get(vertex_label_key, props.get("label", str(vid)))
                    if label is None or str(label).strip() == "":
                        label = str(vid)
                    coords = TGraph.Coordinates(graph, idx, default=[None, None, None])
                    x, y, z = coords if isinstance(coords, list) and len(coords) >= 3 else [None, None, None]
                    degree = len(TGraph.AdjacentVertices(graph, idx, mode="all") or [])
                ...
                for e in edges:
                    ...
                    dst = index_to_id.get(dst_index, None)
                    label = props.get(edge_label_key, props.get("label", "connect"))
                    props = GraphRAG._ensure_ontology_props(
                        grag,
                        props,
                        entity="edge",
                ...
                if not effective_silent:
                    print(f"GraphRAG.SummarizeGraph - Error summarising TGraph: {e}. Returning empty summary.")
        ...
        try:
            ...
            for e in edges:
                ...
                try:
                    ...
                    si = Vertex.Index(vertex=sv, vertices=vertices, strict=False, tolerance=tolerance)
                    ei = Vertex.Index(vertex=ev, vertices=vertices, strict=False, tolerance=tolerance)
    ...
    @staticmethod
    def _graph_vertices(graph) -> List[Any]:
        """
        Returns graph vertices from either a legacy Graph or a TGraph.
        """
        if GraphRAG._is_tgraph(graph):
            try:
                from topologicpy.TGraph import TGraph
    ...
    @staticmethod
    def _graph_edges(graph) -> List[Any]:
        """
        Returns graph edges from either a legacy Graph or a TGraph.
        """
        if GraphRAG._is_tgraph(graph):
            try:
                from topologicpy.TGraph import TGraph
    ...
    @staticmethod
    def _vertex_value(vertex, key, default=None):
        """
        Returns a dictionary value from a TGraph vertex record or Topologic vertex.
        """
        d = GraphRAG._record_dictionary(vertex)
        return d.get(key, default)

    @staticmethod
    def _edge_value(edge, key, default=None):
        """
        Returns a dictionary value from a TGraph edge record or Topologic edge.
        """
        d = GraphRAG._record_dictionary(edge)
        return d.get(key, default)

    @staticmethod
    def _vertex_index(graph, vertex) -> Optional[int]:
        """
        Returns the index of a vertex in either a legacy Graph or a TGraph.
        """
        if GraphRAG._is_tgraph(graph):
            try:
                from topologicpy.TGraph import TGraph
                return TGraph.VertexIndex(graph, vertex)
            except Exception:
                if isinstance(vertex, dict) and isinstance(vertex.get("index"), int):
                    return vertex.get("index")
                return None
        try:
            from topologicpy.Vertex import Vertex
            return Vertex.Index(vertex=vertex, vertices=GraphRAG._graph_vertices(graph), strict=False, tolerance=0.0001)
        except Exception:
            return None

    @staticmethod
    def _graph_degree(graph, vertex) -> int:
        """
        Returns the degree of a vertex in either a legacy Graph or a TGraph.
        """
        if GraphRAG._is_tgraph(graph):
            try:
                from topologicpy.TGraph import TGraph
                idx = TGraph.VertexIndex(graph, vertex)
                if idx is None and isinstance(vertex, dict):
                    idx = vertex.get("index")
                return len(TGraph.AdjacentVertices(graph, idx, mode="all") or [])
            except Exception:
                return 0
        try:
            from topologicpy.Graph import Graph
            return len(Graph.AdjacentVertices(graph, vertex) or [])
        except Exception:
            return 0
    ...
    def unique_id(graph=None, prefix: str = "n", key: str = "id", silent: bool = False) -> str:
        ...
        try:
            ...
            if graph is not None:
                for v in GraphRAG._graph_vertices(graph):
                    value = GraphRAG._vertex_value(v, key, None)
    ...
    def _apply_add_node(grag, graph, action: Dict[str, Any], silent: bool = False) -> Dict[str, Any]:
        try:
            ...
            if GraphRAG._is_tgraph(graph):
                from topologicpy.TGraph import TGraph
                idx = graph.AddVertex(dictionary=props)
                vertex = TGraph.Vertex(graph, idx)
            else:
                from topologicpy.Vertex import Vertex
                from topologicpy.Graph import Graph
                from topologicpy.Topology import Topology
            ...
            if attach_vertex is not None:
                ...
                if GraphRAG._is_tgraph(graph):
                    from topologicpy.TGraph import TGraph
                    a_idx = TGraph.VertexIndex(graph, attach_vertex)
                    b_idx = TGraph.VertexIndex(graph, vertex)
                    if a_idx is not None and b_idx is not None:
                        graph.AddEdge(a_idx, b_idx, dictionary=edge_props)
                else:
    ...
    def _apply_connect(grag, graph, action: Dict[str, Any], silent: bool = False) -> Dict[str, Any]:
        try:
            ...
            if GraphRAG._is_tgraph(graph):
                from topologicpy.TGraph import TGraph
                a_idx = TGraph.VertexIndex(graph, a)
                b_idx = TGraph.VertexIndex(graph, b)
                if a_idx is None or b_idx is None:
                    return {"ok": False, "graph": graph, "message": "Could not resolve both TGraph vertex indices."}
                edge_index = graph.AddEdge(a_idx, b_idx, dictionary=edge_props)
                if edge_index is None:
                    return {"ok": False, "graph": graph, "message": "Could not create edge."}
                return {"ok": True, "graph": graph, "edge": TGraph.Edge(graph, edge_index), "message": "Connected vertices."}
    ...
    def _find_vertex_by_id(grag, graph, value):
        ...
        try:
            for v in GraphRAG._graph_vertices(graph):
                if str(GraphRAG._vertex_value(v, key, "")) == needle:
                    return v
        except Exception:
            return None
        return None
    ...
    def _find_vertex_by_label(grag, graph, value):
        ...
        try:
            for v in GraphRAG._graph_vertices(graph):
                if str(GraphRAG._vertex_value(v, key, "")).lower() == needle:
                    return v
        except Exception:
            return None
        return None
    ...
    def _can_add_connection(grag, graph, vertex, silent: bool = False) -> bool:
        try:
            ...
            if graphdb is None:
                return True
            label = GraphRAG._vertex_value(vertex, getattr(grag, "vertexLabelKey", "label"), None)
            if label is None:
                return True
            current_degree = GraphRAG._graph_degree(graph, vertex)
    ...
    def _degree_limit_message(grag, graph, vertex) -> str:
        try:
            from topologicpy.GraphDB import GraphDB
            label = GraphRAG._vertex_value(vertex, getattr(grag, "vertexLabelKey", "label"), "Unknown")
            current_degree = GraphRAG._graph_degree(graph, vertex)
            max_degree = None
            graphdb = getattr(grag, "graphdb", None)
    ...
    def _matrix_materialise_graph(grag, state: Dict[str, Any], silent: bool = False):
        ...
        edge dictionaries. TGraph is used by default because it stores incidence
        independently from geometric coincidence and can still materialise
        Topologic geometry through TGraph.Topology when needed.
        """
        try:
            from topologicpy.TGraph import TGraph
            ...
            for i, node in enumerate(nodes):
                ...
                graph.AddVertex(dictionary=props)
            ...
            for i in range(len(nodes)):
                for j in range(i + 1, len(nodes)):
                    ...
                    graph.AddEdge(i, j, dictionary=props)
