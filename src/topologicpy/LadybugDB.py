...
def _graph_dictionary(graph: Any) -> Dict[str, Any]:
    """Returns the graph dictionary as a Python dictionary for Graph or TGraph."""
    if graph is None:
        return {}
    if _is_tgraph(graph):
        try:
            ...
            try:
                return dict(graph._dictionary or {})
            except Exception:
                return {}
    try:
...
def _graph_uuid(graph: Any, fallback: str = None) -> str:
    """Returns a UUID-like graph id for Graph or TGraph."""
    try:
        from topologicpy.Topology import Topology
        value = Topology.UUID(graph, silent=True)
    ...
    try:
        ...
        return fallback or "graph"


def _graph_mesh_data(graph: Any, mantissa: int = 6, tolerance: float = 0.0001) -> Dict[str, Any]:
    """
    Returns canonical mesh-like graph data for either Graph or TGraph.
    ...
    if _is_tgraph(graph):
        ...
        md = None
        try:
            md = TGraph.MeshData(graph, mantissa=mantissa, tolerance=tolerance)
        except TypeError:
            try:
                md = TGraph.MeshData(graph, mantissa=mantissa)
            except Exception:
                md = None
        except Exception:
            md = None

        if isinstance(md, dict) and md.get("vertices") is not None:
            return {
                "vertices": md.get("vertices", []) or [],
                "edges": md.get("edges", []) or [],
                "vertexDictionaries": md.get("vertexDictionaries", md.get("vertex_dicts", [])) or [],
                "edgeDictionaries": md.get("edgeDictionaries", md.get("edge_dicts", [])) or [],
            }
        ...
        for i, record in enumerate(records):
            ...
            coords = None
            try:
                coords = TGraph.Coordinates(graph, old_index, default=None)
            except Exception:
                coords = None
            if coords is None:
                x = d.get("x", None)
                ...
                if all(isinstance(c, (int, float)) for c in [x, y, z]):
                    coords = [x, y, z]
            if coords is None:
                angle = 2.0 * 3.141592653589793 * float(i) / float(n)
                import math
                coords = [math.cos(angle), math.sin(angle), 0.0]
            index_map[old_index] = len(vertices)
            vertices.append([round(float(coords[0]), mantissa), round(float(coords[1]), mantissa), round(float(coords[2]), mantissa)])
        ...
        for record in TGraph.Edges(graph, asTopologic=False, activeOnly=True) or []:
            ...
            edge_dicts.append(dict(record.get("dictionary", {})))
    ...
    try:
        ...
        md = Graph.MeshData(graph, mantissa=mantissa)
...
def _annotate_graph_for_ontology(graph, ontology: bool = True, generatedBy: str = "LadybugDB.UpsertGraph", silent: bool = True):
    ...
    if _is_tgraph(graph):
        try:
            from topologicpy.TGraph import TGraph
            gdict = dict(TGraph.Dictionary(graph) or {})
            gdict.setdefault("ontology_class", "top:Graph")
            gdict.setdefault("category", "graph")
            if generatedBy is not None:
                gdict.setdefault("generated_by", generatedBy)
            try:
                graph.SetDictionary(gdict)
            except Exception:
                pass
...
class LadybugDB:
    ...
    def UpsertGraph(manager,
        ...
        try:
            ...
            graph_dict = _graph_dictionary(graph)
            ...
            mesh_data = _graph_mesh_data(graph, mantissa=mantissa, tolerance=tolerance)
            ...
            for i, xyz in enumerate(verts):
                ...
                storage_id = raw_vid if str(raw_vid).startswith(f"{gid}:") else f"{gid}:{raw_vid}"
    ...
    def GraphByID(manager, graphID: str, ontology: bool = True, asTGraph: bool = False, silent: bool = False):
        ...
        ontology : bool , optional
            If True, ontology metadata is preserved/annotated where possible. Default is True.
        asTGraph : bool , optional
            If True, returns a topologicpy.TGraph. Otherwise, returns the legacy Graph. Default is False.
        silent : bool , optional
            If True, suppresses warnings/errors. Default is False.

        Returns
        -------
        topologic_core.Graph or topologicpy.TGraph
            The reconstructed graph, or None.
        """
        try:
            manager.ensure_schema()
            graphID = str(graphID)
            ...
            if asTGraph:
                ...
                for row in rows_v:
                    ...
                    for _k in ["ontology_class", "category", "uri", "source", "generated_by", "derived_from", "ifc_class", "ifc_guid"]:
                        if _k not in props and row.get(_k) not in [None, ""]:
                            props[_k] = row.get(_k)
                    props["x"] = float(row.get("x") if row.get("x") is not None else 0.0)
                    props["y"] = float(row.get("y") if row.get("y") is not None else 0.0)
                    props["z"] = float(row.get("z") if row.get("z") is not None else 0.0)
                    id_to_index[row.get("id")] = len(vertex_records)
                    vertex_records.append(props)
                ...
                for row in rows_e:
                    ...
                    for _k in ["ontology_class", "category", "uri", "source", "generated_by", "derived_from", "ifc_class", "ifc_guid"]:
                        if _k not in props and row.get(_k) not in [None, ""]:
                            props[_k] = row.get(_k)
                    props["src"] = id_to_index[a_id]
                    props["dst"] = id_to_index[b_id]
                    edge_records.append(props)

                return TGraph.ByVerticesEdges(vertices=vertex_records, edges=edge_records, dictionary=g_props, ontology=ontology, silent=silent)
        ...
        try:
            ...
            if asTGraph:
                ...
                for nid, row in node_rows.items():
                    ...
                    for key in ["x", "y", "z"]:
                        try:
                            props[key] = float(row.get(key, 0.0))
                        except Exception:
                            props[key] = 0.0
                    id_to_index[nid] = len(vertex_records)
                    vertex_records.append(props)
                ...
                for rel in rel_rows:
                    ...
                    if key in seen:
                        continue
                    seen.add(key)
                    if str(a_id) not in id_to_index or str(b_id) not in id_to_index:
                        continue
                    props = dict(_json_loads(rel.get("props"), {}))
                    if "label" not in props and rel.get("label") is not None:
                        props["label"] = rel.get("label")
                    props["src"] = id_to_index[str(a_id)]
                    props["dst"] = id_to_index[str(b_id)]
                    edge_records.append(props)
                return [TGraph.ByVerticesEdges(vertices=vertex_records, edges=edge_records, ontology=ontology, silent=silent)]
    ...
    @staticmethod
    def GraphsByOntologyClass(manager, ontologyClass: str, ontology: bool = True, asTGraph: bool = False, silent: bool = False):
        """
        Returns TopologicPy Graphs or TGraphs whose stored graph ontology_class matches the input class.
        """
        try:
