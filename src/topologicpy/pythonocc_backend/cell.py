...
class Cell(Topology):
    ...
    def ByWires(
        ...
        The supplied section curves are passed directly to OCCT; they are not
        tessellated or rebuilt. Corresponding section Wires must contain the same
        number of Edges so that OCCT can preserve explicit edge correspondence.
        ...
        close : bool , optional
            Reserved for API compatibility. Closed-loop lofting through the first
            and last sections is not supported by this solid constructor; if True,
            the method returns None. Default is False.
        ...
        if close:
            if not silent:
                print("Cell.ByWires - Error: The close=True mode is not supported for solid lofts. Returning None.")
            return None
        ...
        if len(wire_list) < 2:
            if not silent:
                print("Cell.ByWires - Error: At least two valid section Wires are required. Returning None.")
            return None
        ...
        if not math.isfinite(tolerance) or tolerance <= 0.0:
            if not silent:
                print("Cell.ByWires - Error: The tolerance parameter must be a finite positive number. Returning None.")
            return None
        ...
        try:
            ...
            if not silent:
                print("Cell.ByWires - Error: Could not import the required OpenCascade loft classes. Returning None.")
            return None
        ...
        for wire in wire_list:
            ...
            if expected_edge_count is None:
                ...
                if not silent:
                    print("Cell.ByWires - Error: Corresponding Wires must contain the same number of Edges. Returning None.")
                return None
        ...
        try:
            ...
            if not silent:
                print("Cell.ByWires - Error: Native OCCT loft construction failed. Returning None.")
            return None

        if _is_null_shape(shape):
            return None

        result = Cell._native_result(shape)
        if not isinstance(result, Cell) and not silent:
            print("Cell.ByWires - Error: The loft did not produce a valid Cell. Returning None.")
